...
- Image paths and alt text are centralized in `src/lib/images.ts`; tests assert specific alt copy (e.g., `MenuItemCard` uses `Old Crown Girton`).
- App-wide metadata and email branding rely on `config.ts`, `src/lib/content/environment.ts`, and `public/sw.js` comment headers.
- Content pages (blog, events, legal, offline/not-found) embed venue name/location strings directly; several tests in `__tests__` and `tests/` assert the Old Crown copy.
...
- Site uses DaisyUI/Tailwind; no global find/replace without checking components using markup like `Old Crown Girton` (e.g., alt text with location context) to avoid regressions.
...
- Audit `public/images` for Old Crown-specific assets; if no White Horse replacements exist yet, keep filenames but adjust alt copy while queuing asset rename separately.
