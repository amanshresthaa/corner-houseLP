...
function reportHardcodedColors(files) {
  ...
  files.forEach(file => {
    const content = fs.readFileSync(file, 'utf8');
    if (HEX_REGEX.test(content) || RGB_REGEX.test(content)) {
      offenders.push(file);
    }
  });

  if (offenders.length > 0) {
    console.log('\n⚠️  Hardcoded color literals detected in:');
    offenders.slice(0, 20).forEach(file => console.log(`   - ${file}`));
    if (offenders.length > 20) {
      console.log(`   ...and ${offenders.length - 20} more files`);
    }
    console.log('   Extend COLOR_MIGRATIONS or refactor these files manually.\n');
  } else {
    console.log('\n✅ No hardcoded hex/rgb literals detected in scanned files.\n');
  }
}
...
function main() {
  ...
  validateMigrations();
  reportHardcodedColors(targetFiles);
