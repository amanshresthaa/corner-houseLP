...
export default function SocialMediaSection({
  ...
  const instagram = social.instagram?.url ?? '#';
  const facebookLabel = social.facebook?.handle ?? 'Facebook';
  const instagramLabel = social.instagram?.handle ?? 'Instagram';
    ...
    <div className={`bg-neutral-50 p-6 rounded-xl shadow-lg text-center ${className}`}>
      ...
      <div className="flex justify-center space-x-6">
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          aria-label={`Follow us on ${instagramLabel}`}
        >
          <svg className="h-8 w-8" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
