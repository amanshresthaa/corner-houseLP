...
interface AboutWelcomeSectionProps {
  content: AboutWelcomeContent;
  className?: string;
}

export default function AboutWelcomeSection({ content, className = '' }: AboutWelcomeSectionProps) {
  const { heading, tagline, paragraphs = [], features = [], image } = content || {} as AboutWelcomeContent;
  if (!heading) return null;

  return (
    <section className={`py-14 sm:py-16 ${className}`} aria-labelledby="about-welcome-heading">
        ...
        <div className="rounded-3xl border border-brand-100/70 bg-white shadow-xl">
          <div className="grid gap-8 p-6 md:p-10 lg:grid-cols-12 lg:items-center">
            <div className="lg:col-span-7">
              ...
              <h2 id="about-welcome-heading" className="mt-3 text-3xl sm:text-4xl font-display font-bold text-stout-700">
                {heading}
              </h2>
              {tagline ? (
                <p className="mt-2 text-base sm:text-lg font-semibold text-accent-700">
                  {tagline}
                </p>
              ) : null}
              ...
              {paragraphs.length > 0 ? (
                <div className="prose prose-brand mt-5 max-w-none text-brand-800">
                  {paragraphs.map((p, i) => (
                    <p key={`about-welcome-paragraph-${i}`}>{p}</p>
                  ))}
                </div>
              ) : null}
              ...
              {features.length > 0 ? (
                <div className="mt-6 rounded-2xl bg-brand-50/80 p-5 ring-1 ring-brand-100">
                  ...
                  <ul className="mt-4 grid gap-3 sm:grid-cols-2">
                    {features.map((f, i) => (
                      <li key={`about-feature-${i}`} className="flex items-start gap-3 rounded-xl bg-white p-3 text-sm font-medium text-brand-700 shadow-sm ring-1 ring-brand-100">
