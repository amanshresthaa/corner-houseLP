							...
							{currentButtonIndex === 0 && (
								...
								>
									...
									{(() => {
										...
										const href = isApple ? appleHref : googleHref; // DOM-facing href (https) so tests can read it
										...
										return (
											<a
												...
												onClick={(e) => {
													// Attempt native maps on Apple devices immediately
													if (isApple) {
														e.preventDefault();
														try {
															window.location.href = appleNative;
															// Fallback to https appleHref after a short delay in case native scheme isn't handled
															window.setTimeout(() => window.open(appleHref, '_blank', 'noopener'), 1500);
														} catch (err) {
															window.open(appleHref, '_blank', 'noopener');
														}
													} else {
														// non-Apple: let the anchor navigate (opens Google Maps URL)
													}
													track('directions_click', { platform: isApple ? 'apple' : 'other', href });
												}}
											>
