...
import { z } from 'zod';
import { deepMergeContent, wrapWithMountPath } from '@/src/lib/content/utils';
  ...
  try {
    ...
    for (const fileName of moduleConfig.files) {
      ...
      try {
        ...
        composedData = deepMergeContent(composedData, fileData, 'concat');
      } catch (fileError) {
    ...
    if (envConfig?.overrides) {
      for (const overridePath of envConfig.overrides) {
        ...
        try {
          ...
          composedData = deepMergeContent(composedData, overrideData, 'concat');
        } catch {
    ...
    const mountedData = wrapWithMountPath(
      composedData,
      moduleConfig.mountPath || moduleId.replace(/\//g, '.')
    );
